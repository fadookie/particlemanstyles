...
public class ExampleParticleStyleLineMove implements ParticleStyle {

    private int step = 0;
    private final int maxStep = 20;
    private final double radius = 1.25;
    private final int amount = 5;
    private final double spread = 0.0;
    ...
    public List<PParticle> getParticles(ParticlePair particle, Location location) {
        ...
        double z = 0;//Math.sin(angle) * this.radius;
        Location point = location.clone().add(x, 0, z);

        for (int i = 0; i < this.amount; i++)
            particles.add(new PParticle(location, x, 0, z, this.speed));
